...
public class Lab {
	...
	public static void main(String[] args) {
		...
		List<Book> lengthyBooks = allBooks.stream().filter(b -> b.getPages() > 400).collect(toList());

//			lengthyBooks.forEach(b -> System.out.println(b));

		// 2. Find all books that are java books and more then 400 pages
		List<Book> javaBooks = allBooks.stream().filter(b -> b.getPages() > 400 && b.getSubject() == Subject.JAVA)
				.collect(toList());
...
//			javaBooks.forEach(b -> System.out.println(b));
		...
		List<Book> top3Books = allBooks.stream().sorted((o1, o2) -> Integer.compare(o2.getPages(), o1.getPages()))
				.limit(3).collect(toList());

//			top3Books.forEach(b -> System.out.println(b));

		// 4. We need from the fourth to the last longest books
		List<Book> skip3Books = allBooks.stream().sorted((o1, o2) -> Integer.compare(o1.getPages(), o2.getPages()))
...
//			skip3Books.forEach(b -> System.out.println(b));
		...
		List<Integer> publishingYears = allBooks.stream().map(b -> b.getYear()).collect(toList());

//			publishingYears.forEach(b -> System.out.println(b));

		// 6. We need all the authors names who have written a book

		List<String> authorNames = allBooks.stream().flatMap(b -> b.getAuthors().stream()).map(a -> a.getName())
...
//			authorNames.forEach(b -> System.out.println(b));
		...
		boolean moreThanOneAuthor = allBooks.stream().allMatch(b -> b.getAuthors().size() > 1);

//			System.out.println(moreThanOneAuthor);

		// We want one of the books written by more than one author.? (findAny)
		Optional<Book> oneBook = allBooks.stream().parallel().filter(b -> b.getAuthors().size() > 1).findAny();

//			System.out.println(oneBook.get());

		// We want to know how many pages the longest book has.

		Optional<Integer> longestBook = allBooks.stream().map(b -> b.getPages()).reduce(Integer::max);

//			System.out.println(longestBook.get());

		// We want the average number of pages of the books
		Double averagePages = allBooks.stream().collect(Collectors.averagingInt(b -> b.getPages()));

//			System.out.println(averagePages);

		// We want all the titles of the books
		String allTitles = allBooks.stream().map(b -> b.getTitle()).collect(Collectors.joining(" "));

//			System.out.println(allTitles);

		// We want the book with the higher number of authors?
		Optional<Book> bookWrittenByHighestAuthors = allBooks.stream()
...
//			System.out.println(bookWrittenByHighestAuthors.get());
		...
		Map<Integer, List<Book>> bookPerYear = allBooks.stream().collect(Collectors.groupingBy(b -> b.getYear()));

//			System.out.println(bookPerYear);

		// We want a Map of book per year and then by subject
		Map<Object, Map<Object, List<Book>>> bookPerYearPerTttle = allBooks.stream()
